...
function ReceitasEDespesas(){
	...
	const exportarExcel = () => {
		const data = [ ['Data','Descrição','Tipo','Valor'], ...items.map(i=>[
			i.data.toLocaleDateString(), i.desc, i.tipo, (i.tipo==='Receita'?'+':'-')+` R$ ${i.valor.toFixed(2).replace('.',',')}`
		])]
		const ws = XLSX.utils.aoa_to_sheet(data)
		const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, 'Transações')
